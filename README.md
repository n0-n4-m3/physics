...
*Сила трения*:
![ea](https://latex.codecogs.com/svg.image?F\textsc{friction}&space;=&space;\mu&space;N), здесь:
* μ – коэффициент трения – указывает на отношение между трением и силой, прижимающей тело к опоре;
* N – сила реакции нитки, опоры.

*Момент силы*:
![eq](https://latex.codecogs.com/svg.image?M&space;=&space;Fd) или ![ea](https://latex.codecogs.com/svg.image?M&space;=&space;Fl), здесь:
...
*Момент инерции*:
![eq](https://latex.codecogs.com/svg.image?J&space;=&space;mr^2), здесь:
* m – масса тела;
* r – радиус.

*Давление, оказываемое телом на опору: нитку, поверхность*:
![ea](https://latex.codecogs.com/svg.image?p&space;=&space;\tfrac{F}{S}),
* F – сила;
* S – площадь контакта.

*Правило рычага*:
![wa](https://latex.codecogs.com/svg.image?\frac{F\textsc{1}}{F\textsc{2}}&space;=&space;\frac{d\textsc{1}}{d\textsc{2}}), здесь:
